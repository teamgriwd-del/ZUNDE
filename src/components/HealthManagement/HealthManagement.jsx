...
import { Pill, Calculator, AlertCircle, History, ShieldCheck, ChevronRight } from 'lucide-react';
    ...
    <div className="zunde-health-mgmt enterprise p-8 bg-gray-50 h-full overflow-y-auto">
      <div className="health-header flex justify-between items-center mb-8 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
        <div>
            ...
            <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Lifecycle Protocols • Dosage Intelligence</p>
        </div>
      ...
      {!selectedAnimal ? (
        <div className="empty-state py-20 bg-white rounded-[40px] border-2 border-dashed border-gray-200 text-center flex flex-col items-center">
        ...
        <div className="dashboard-grid grid grid-cols-2 gap-8 text-left">
          ...
          {/* Medication Intelligence Card (THE DEEP MODULE) */}
          <div className="health-card bg-white p-8 rounded-[40px] shadow-sm border border-gray-100">
            ...
            <div className="med-calc-box bg-blue-50/50 p-6 rounded-[30px] border border-blue-100/50">
                ...
                <select className="w-full p-4 bg-white rounded-2xl font-bold border-none outline-none shadow-sm mb-6" value={selectedMeds} onChange={e => setSelectedMeds(e.target.value)}>
                ...
                <div className="dosage-result flex items-center justify-between mb-6">
                    ...
                    <div className="text-right">
                        <span className="text-[10px] font-black text-blue-900/40 uppercase block">Frequency</span>
                        <strong className="text-sm font-black text-blue-600">{DOSAGE_RATES[selectedMeds].frequency}</strong>
                    </div>
                </div>

                <p className="text-xs font-bold text-blue-900/60 leading-relaxed bg-white/50 p-4 rounded-2xl border border-blue-100/50 italic">
                    <AlertCircle size={12} className="inline mr-1 mb-0.5" /> Note: {DOSAGE_RATES[selectedMeds].note} Dose calculated for current weight of {selectedAnimal.currentWeight}kg.
                </p>
            </div>

            <div className="compliance-meter mt-8 pt-8 border-t border-gray-50 text-center">
                <div className="flex justify-between items-center mb-4">
                    <span className="text-sm font-black text-gray-800">Protocol Compliance</span>
                    <span className="text-sm font-black text-zunde-green">High</span>
                </div>
                <div className="w-full h-3 bg-gray-100 rounded-full overflow-hidden">
                    <div className="bg-zunde-green h-full w-[92%]" style={{boxShadow: '0 0 10px rgba(46,125,50,0.3)'}}></div>
                </div>
            </div>
          </div>

          {/* Vaccination Compliance Card */}
          <div className="health-card bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 col-span-2">
                ...
                <div key={i} className={`p-6 rounded-[30px] border-2 transition flex items-center justify-between ${task.status === 'Completed' ? 'bg-green-50 border-green-100 opacity-60' : (task.status === 'Overdue' ? 'bg-red-50 border-red-100 animate-pulse' : 'bg-gray-50 border-gray-100')}`}>
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{task.dueDate}</span>
                    </div>
                  </div>
